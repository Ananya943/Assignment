...
def clean_company_sales(df):
    """Clean company sales data according to requirements"""
    ...
    df_cleaned = df_cleaned.fillna({"units_sold": 0, "revenue": 0.0})
    
    # Remove duplicates based on item_id
    df_cleaned = df_cleaned.dropDuplicates(["item_id"])
...
def main(config_path):
    ...
    input_path = config['company_sales']['input_path']
    output_path = config['company_sales']['hudi_output_path']
    ...
    try:
        ...
        # 4. Clean data
        df_cleaned = clean_company_sales(df)
        ...
        if quarantine_df.count() > 0:
            quarantine_path = "/app/data/quarantine/company_sales_quarantine"
        ...
        if good_df.count() > 0:
            ...
            write_to_hudi(good_df, output_path, "company_sales")
